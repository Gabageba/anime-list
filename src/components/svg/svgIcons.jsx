import React from 'react'

export const SettingsSvg = () => {
  return (
    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path
        d="M10 6C11.0904 6 12.1361 6.42143 12.9071 7.17157C13.6781 7.92172 14.1113 8.93913 14.1113 10C14.1113 11.0609 13.6781 12.0783 12.9071 12.8284C12.1361 13.5786 11.0904 14 10 14C8.90962 14 7.8639 13.5786 7.09289 12.8284C6.32187 12.0783 5.88872 11.0609 5.88872 10C5.88872 8.93913 6.32187 7.92172 7.09289 7.17157C7.8639 6.42143 8.90962 6 10 6ZM10 8C9.45481 8 8.93195 8.21071 8.54644 8.58579C8.16094 8.96086 7.94436 9.46957 7.94436 10C7.94436 10.5304 8.16094 11.0391 8.54644 11.4142C8.93195 11.7893 9.45481 12 10 12C10.5452 12 11.068 11.7893 11.4536 11.4142C11.8391 11.0391 12.0556 10.5304 12.0556 10C12.0556 9.46957 11.8391 8.96086 11.4536 8.58579C11.068 8.21071 10.5452 8 10 8ZM7.94436 20C7.6874 20 7.47156 19.82 7.43045 19.58L7.05016 16.93C6.40263 16.68 5.84761 16.34 5.31314 15.94L2.75387 16.95C2.52775 17.03 2.25024 16.95 2.1269 16.73L0.0712604 13.27C-0.0623562 13.05 -0.000686894 12.78 0.194599 12.63L2.3633 10.97L2.29135 10L2.3633 9L0.194599 7.37C-0.000686894 7.22 -0.0623562 6.95 0.0712604 6.73L2.1269 3.27C2.25024 3.05 2.52775 2.96 2.75387 3.05L5.31314 4.05C5.84761 3.66 6.40263 3.32 7.05016 3.07L7.43045 0.42C7.47156 0.18 7.6874 0 7.94436 0H12.0556C12.3126 0 12.5284 0.18 12.5695 0.42L12.9498 3.07C13.5974 3.32 14.1524 3.66 14.6869 4.05L17.2461 3.05C17.4722 2.96 17.7498 3.05 17.8731 3.27L19.9287 6.73C20.0624 6.95 20.0007 7.22 19.8054 7.37L17.6367 9L17.7086 10L17.6367 11L19.8054 12.63C20.0007 12.78 20.0624 13.05 19.9287 13.27L17.8731 16.73C17.7498 16.95 17.4722 17.04 17.2461 16.95L14.6869 15.95C14.1524 16.34 13.5974 16.68 12.9498 16.93L12.5695 19.58C12.5284 19.82 12.3126 20 12.0556 20H7.94436ZM9.22913 2L8.84884 4.61C7.61546 4.86 6.52597 5.5 5.73455 6.39L3.2575 5.35L2.48664 6.65L4.65534 8.2C4.24421 9.37 4.24421 10.64 4.65534 11.8L2.47636 13.36L3.24722 14.66L5.74483 13.62C6.53625 14.5 7.61546 15.14 8.83856 15.38L9.21886 18H10.7811L11.1614 15.39C12.3845 15.14 13.4638 14.5 14.2552 13.62L16.7528 14.66L17.5236 13.36L15.3447 11.81C15.7558 10.64 15.7558 9.37 15.3447 8.2L17.5134 6.65L16.7425 5.35L14.2655 6.39C13.474 5.5 12.3845 4.86 11.1512 4.62L10.7709 2H9.22913Z"
        fill="#fff"
      />
    </svg>
  )
}

export const NotificationSvg = () => {
  return (
    <svg width="18" height="20" viewBox="0 0 18 20" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path
        d="M6.66667 18.0952H10.4762C10.4762 19.1429 9.61905 20 8.57143 20C7.52381 20 6.66667 19.1429 6.66667 18.0952ZM17.1429 16.1905V17.1429H0V16.1905L1.90476 14.2857V8.57143C1.90476 5.61905 3.80952 3.04762 6.66667 2.19048V1.90476C6.66667 0.857143 7.52381 0 8.57143 0C9.61905 0 10.4762 0.857143 10.4762 1.90476V2.19048C13.3333 3.04762 15.2381 5.61905 15.2381 8.57143V14.2857L17.1429 16.1905ZM13.3333 8.57143C13.3333 5.90476 11.2381 3.80952 8.57143 3.80952C5.90476 3.80952 3.80952 5.90476 3.80952 8.57143V15.2381H13.3333V8.57143Z"
        fill="#fff"
      />
    </svg>
  )
}

export const BookmarksOutlineSvg = () => {
  return (
    <svg width="14" height="18" viewBox="0 0 14 18" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path
        d="M12 0H2C0.9 0 0.0100002 0.9 0.0100002 2L0 18L7 15L14 18V2C14 0.9 13.1 0 12 0ZM12 15L7 12.82L2 15V2H12V15Z"
        fill="#fff"
      />
    </svg>
  )
}

export const HomeOutlineSvg = () => {
  return (
    <svg width="16" height="18" viewBox="0 0 16 18" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M5 10H11V16H14V7L8 2.5L2 7V16H5V10ZM0 18V6L8 0L16 6V18H0Z" fill="#fff"/>
    </svg>
  )
}

export const ProfileOutlineSvg = () => {
  return (
    <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path
        d="M9 0C4.032 0 0 4.032 0 9C0 13.968 4.032 18 9 18C13.968 18 18 13.968 18 9C18 4.032 13.968 0 9 0ZM4.815 14.85C5.994 14.004 7.434 13.5 9 13.5C10.566 13.5 12.006 14.004 13.185 14.85C12.006 15.696 10.566 16.2 9 16.2C7.434 16.2 5.994 15.696 4.815 14.85ZM14.526 13.608C13.005 12.42 11.088 11.7 9 11.7C6.912 11.7 4.995 12.42 3.474 13.608C2.43 12.357 1.8 10.755 1.8 9C1.8 5.022 5.022 1.8 9 1.8C12.978 1.8 16.2 5.022 16.2 9C16.2 10.755 15.57 12.357 14.526 13.608Z"
        fill="#fff"
      />
      <path
        d="M9.00001 3.60001C7.26301 3.60001 5.85001 5.01301 5.85001 6.75001C5.85001 8.48701 7.26301 9.90001 9.00001 9.90001C10.737 9.90001 12.15 8.48701 12.15 6.75001C12.15 5.01301 10.737 3.60001 9.00001 3.60001ZM9.00001 8.10001C8.25301 8.10001 7.65001 7.49701 7.65001 6.75001C7.65001 6.00301 8.25301 5.40001 9.00001 5.40001C9.74701 5.40001 10.35 6.00301 10.35 6.75001C10.35 7.49701 9.74701 8.10001 9.00001 8.10001Z"
        fill="#fff"
      />
    </svg>
  )
}

export const AnimeOutlineSvg = () => {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 16">
      <g>
        <path
          d="M2,2.47l1.76,3.53h14.24V14H2V2.47M20,0h-4l2,4h-3L13,0h-2l2,4h-3L8,0h-2l2,4h-3L3,0h-1C.9,0,0,.9,0,2V14c0,1.1,.89,2,1.99,2H18c1.1,0,2-.9,2-2V0Z"/>
      </g>
    </svg>
  )
}

export const MangaOutlineSvg = () => {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 319.94 248.68">
      <g>
        <path
          d="M290.85,7.27c-16-5.82-33.45-7.27-50.9-7.27-27.63,0-58.17,5.82-79.98,21.81C138.16,5.82,107.62,0,79.98,0,62.53,0,45.08,1.45,29.09,7.27,17.45,10.18,8.73,14.54,0,21.81V234.14c0,2.91,2.91,7.27,7.27,7.27h4.36c20.36-11.63,45.08-14.54,69.8-14.54s58,8.33,78.53,20.91v.91c.24-.15,.49-.3,.73-.45,.24,.15,.49,.3,.73,.45v-.91c20.53-12.58,54.38-20.91,78.53-20.91s49.44,2.91,69.8,14.54h2.91c4.36,0,7.27-4.36,7.27-7.27V21.81c-8.73-7.27-17.45-11.63-29.09-14.54Zm0,196.33c-16-5.82-33.45-7.27-50.9-7.27-24.43,0-60.22,9.95-79.31,22.81-21.86-12.87-56.23-22.81-80.66-22.81-17.45,0-34.9,1.45-50.9,7.27V36.36c16-5.82,33.45-7.27,50.9-7.27,24.72,0,59.62,8.73,79.98,21.81h0s0,0,0,0h0c20.36-13.09,55.26-21.81,79.98-21.81,17.45,0,34.9,1.45,50.9,7.27V203.6Z"/>
      </g>
    </svg>
  )
}

export const FavouriteOutlineSvg = () => {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 18.35">
      <g>
        <path
          d="M14.5,0c-1.74,0-3.41,.81-4.5,2.09C8.91,.81,7.24,0,5.5,0,2.42,0,0,2.42,0,5.5c0,3.78,3.4,6.86,8.55,11.54l1.45,1.31,1.45-1.32c5.15-4.67,8.55-7.75,8.55-11.53,0-3.08-2.42-5.5-5.5-5.5Zm-4.4,15.55l-.1,.1-.1-.1C5.14,11.24,2,8.39,2,5.5c0-2,1.5-3.5,3.5-3.5,1.54,0,3.04,.99,3.57,2.36h1.87c.52-1.37,2.02-2.36,3.56-2.36,2,0,3.5,1.5,3.5,3.5,0,2.89-3.14,5.74-7.9,10.05Z"/>
      </g>
    </svg>
  )
}

export const BookmarksFilledSvg = () => {
  return (
    <svg width="14" height="18" viewBox="0 0 14 18" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path
        d="M12 0H2C0.9 0 0.0100002 0.9 0.0100002 2L0 18L7 15L14 18V2C14 0.9 13.1 0 12 0Z"
        fill="#fff"
      />
    </svg>
  )
}

export const HomeFilledSvg = () => {
  return (
    <svg width="16" height="18" viewBox="0 0 16 18" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M8 0L16 6V18H11V11H5V18H0V6L8 0Z" fill="white"/>
    </svg>
  )
}

export const ProfileFilledSvg = () => {
  return (
    <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path
        d="M9 0C4.032 0 0 4.032 0 9C0 13.968 4.032 18 9 18C13.968 18 18 13.968 18 9C18 4.032 13.968 0 9 0ZM9 3.6C10.737 3.6 12.15 5.013 12.15 6.75C12.15 8.487 10.737 9.9 9 9.9C7.263 9.9 5.85 8.487 5.85 6.75C5.85 5.013 7.263 3.6 9 3.6ZM9 16.2C7.173 16.2 5.013 15.462 3.474 13.608C4.995 12.42 6.912 11.7 9 11.7C11.088 11.7 13.005 12.42 14.526 13.608C12.987 15.462 10.827 16.2 9 16.2Z"
        fill="#828282"
      />
    </svg>
  )
}

export const AnimeFilledSvg = () => {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 16">
      <g>
        <path
          d="M16,0l2,4h-3L13,0h-2l2,4h-3L8,0h-2l2,4h-3L3,0h-1C.9,0,0,.9,0,2V14c0,1.1,.89,2,1.99,2H18c1.1,0,2-.9,2-2V0h-4Z"/>
      </g>
    </svg>
  )
}

export const MangaFilledSvg = () => {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 155.45">
      <g>
        <path
          d="M181.82,4.55c-10-3.64-20.91-4.55-31.82-4.55-17.27,0-36.36,3.64-50,13.64C87.27,3.64,68.18,0,50,0c-10.91,0-21.82,.91-31.82,4.55C10.91,6.36,5.45,9.09,0,13.64V146.36c0,1.82,1.82,4.55,4.55,4.55h1.82c12.73-7.27,28.18-9.09,43.64-9.09s37.27,5.45,50,13.64c12.73-8.18,34.55-13.64,50-13.64s30.91,1.82,43.64,9.09h1.82c2.73,0,4.55-2.73,4.55-4.55V13.64c-5.45-4.55-10.91-7.27-18.18-9.09Z"/>
      </g>
    </svg>
  )
}

export const FavouriteFilledSvg = () => {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 18.35">
      <g>
        <path
          d="M10,18.35l-1.45-1.32C3.4,12.36,0,9.28,0,5.5,0,2.42,2.42,0,5.5,0c1.74,0,3.41,.81,4.5,2.09,1.09-1.28,2.76-2.09,4.5-2.09,3.08,0,5.5,2.42,5.5,5.5,0,3.78-3.4,6.86-8.55,11.54l-1.45,1.31Z"/>
      </g>
    </svg>
  )
}

export const SearchSvg = () => {
  return (
    <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path
        d="M12.8645 11.3208H12.0515L11.7633 11.0429C12.7719 9.86964 13.3791 8.34648 13.3791 6.68954C13.3791 2.99485 10.3842 0 6.68954 0C2.99485 0 0 2.99485 0 6.68954C0 10.3842 2.99485 13.3791 6.68954 13.3791C8.34648 13.3791 9.86964 12.7719 11.0429 11.7633L11.3208 12.0515V12.8645L16.4666 18L18 16.4666L12.8645 11.3208ZM6.68954 11.3208C4.12693 11.3208 2.05832 9.25214 2.05832 6.68954C2.05832 4.12693 4.12693 2.05832 6.68954 2.05832C9.25214 2.05832 11.3208 4.12693 11.3208 6.68954C11.3208 9.25214 9.25214 11.3208 6.68954 11.3208Z"
        fill="#828282"
      />
    </svg>
  )
}

export const MoreSvg = () => {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 4 16">
      <g>
        <path
          d="M2,4c1.1,0,2-.9,2-2S3.1,0,2,0,0,.9,0,2s.9,2,2,2Zm0,2c-1.1,0-2,.9-2,2s.9,2,2,2,2-.9,2-2-.9-2-2-2Zm0,6c-1.1,0-2,.9-2,2s.9,2,2,2,2-.9,2-2-.9-2-2-2Z"/>
      </g>
    </svg>
  )
}

export const RatingSvg = () => {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24px"
         fill="#000000">
      <g>
        <path d="M0 0h24v24H0V0z" fill="none"/>
        <path d="M0 0h24v24H0V0z" fill="none"/>
      </g>
      <g>
        <path d="M12 17.27 18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21 12 17.27z"/>
      </g>
    </svg>
  )
}

export const EditPlaylist = () => {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 18.85 14">
      <g>
        <path
          d="M0,0V2H11V0H0M0,4v2H11v-2H0m17,.1c-.1,0-.3,.1-.4,.2l-1,1,2.1,2.1,1-1c.2-.2,.2-.6,0-.8l-1.3-1.3c-.1-.1-.2-.2-.4-.2m-1.9,1.8l-6.1,6v2.1h2.1l6.1-6.1-2.1-2M0,8v2H7v-2H0Z"/>
      </g>
    </svg>
  )
}



